...
class python_generator:
    ...
    def recursivecodegen(self, msgtogenerate):
        ...
        if (mymessage.type == 'SEQUENCE'):
            ...
            if mymessage.name in self.msggen.failedBITSTRINGS:
                # no occurance in MAPEM usecase. implement if needed!
                raise Exception()
            else:
                # asn1tools converts bytes to tuple(bytes, int) with bytes as 
                # the BIT STRING (left-adjusted) and int with length information.
                self.add('print(mydata)')
                self.add('bitlength = mydata[1]')
                self.add('bitstring = int.from_bytes(mydata[0], "big")')
                self.add('listoftruth = list()')
                self.add('while bitlength:')
                self.dentin()
                self.add('if (bitstring & (1 << ((len(mydata[0])*8)-1))):')
                self.dentin()
                self.add('print("True")')
                self.add('listoftruth.append(1)')
                self.dentout()
                self.add('else:')
                self.dentin()
                self.add('print("False")')
                self.add('listoftruth.append(0)')
                self.dentout()
                self.add('bitstring = bitstring << 1')
                self.add('bitlength = bitlength - 1')
                self.dentout()
                self.add('print(listoftruth)')
                for idx, e in enumerate(mymessage.contentsList):
                    if not e:
                        continue
                    e = e[0]
                    self.add('msg.' + _msg_name_str(e[mymessage._pos_variable] + ' = listoftruth[' + str(idx) + ']') )
